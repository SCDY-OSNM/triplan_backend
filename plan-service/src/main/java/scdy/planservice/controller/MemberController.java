...
@RequestMapping("/api/v1/plans/members")
public class MemberController {
    ...
    @GetMapping("/{planId}")
    public ResponseEntity<ApiResponse<List<MemberResponseDto>>> getMember(@RequestHeader("X-Authenticated-User") Long userId, @PathVariable ("planId") Long planId){
        List<MemberResponseDto> memberResponseDtoList = memberService.readPlanMemberByPlan(planId);
        return ResponseEntity.ok(ApiResponse.success("플랜 멤버 전체 조회 완료", memberResponseDtoList));
    }

    @PutMapping("/{memberId}")
    public ResponseEntity<ApiResponse<MemberResponseDto>> updateMemberRole(@RequestHeader("X-Authenticated-User") Long userId, @PathVariable("memberId") Long memberId, @RequestBody Long planId){
    ...
    public ResponseEntity<ApiResponse<MemberResponseDto>> deleteMember(@RequestHeader("X-Authenticated-User") Long userId, @PathVariable("memberId") Long memberId){
